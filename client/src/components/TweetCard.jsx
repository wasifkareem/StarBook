...
const TweetCard = ({
  ...
  const addToWall = async () => {
    ...
    const res = await axios.put(
      `http://localhost:3001/api/wall/update-wall?spaceId=${spaceId}&testimonialId=${Id}&WOF=true`,
      {}
    );
    if (res.status == 200) {
  ...
  const removeFromWall = async () => {
    setIsWallChange(true);
    const res = await axios.put(
      `http://localhost:3001/api/wall/update-wall?spaceId=${spaceId}&testimonialId=${Id}&WOF=false`,
      {}
    );
    if (res.status == 200) {
      setIsWallChange(false);
  ...
  const handleDelete = async () => {
    ...
    const res = await axios.delete(
      `http://localhost:3001/api/testimonials/delete?spaceId=${spaceId}&testimonialId=${Id}`
    );
    if (res.status == 200) {
      setIsDel(false);
      dispatch(ReloadCards());

...
import { createTestimonialSchema, deleteTestimonialSchema } from "../src/schemas/testimonial.schema.ts";
...
router.get("/fetch-all", validateQuery(spaceQuerySchema), async (req, res) => {
...
router.delete("/delete", validateQuery(deleteTestimonialSchema), async (req, res) => {
    ...
    try {
        const deleteTestimonial = await Space.updateOne({ _id: spaceId }, {
            $pull: {

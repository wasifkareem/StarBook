...
import { spaceQuerySchema } from "../src/schemas/space.schema.ts";
...
router.put("/update-wall", validateQuery(updateWallQuerySchema), async (req, res) => {
    ...
    try {
        const UpdatedSpace = await Space.findOneAndUpdate({ _id: spaceId, "testimonials._id": testimonialId }, { $set: { "testimonials.$.WOF": isActiveBoolean } }, { new: true });
        if (isActiveBoolean) {
            await Space.updateOne({ _id: spaceId }, {
                $push: {
                    WOF: UpdatedSpace?.testimonials.find((t) => t._id.toString() === testimonialId),
                },
            });
        }
        else {
            await Space.updateOne({ _id: spaceId }, { $pull: { WOF: { _id: testimonialId } } });
        }
